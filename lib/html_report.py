...
v8.2 changes vs v8.1:
  ...
  - Private _load_json removed; uses config.load_json instead.
...
def generate_html_report(work_dir, cfg):
    ...
    cards_html = ''.join(
        f'<div class="card"><div class="value">{_esc(str(val))}</div>'
        f'<div class="label">{_esc(lbl)}</div></div>'
        for lbl, val in card_defs
    )

    # ── profile coverage block ─────────────────────────────────────────────────
    _cov_zero   = coverage.get('commits_matched_zero_profiles', '-')
    _cov_single = coverage.get('commits_matched_one_profile', '-')
    _cov_multi  = coverage.get('commits_matched_multiple_profiles', '-')
    cov_html = (
        f'<div class="coverage">'
        f'<h2>Profile Coverage</h2>'
        f'<dl>'
        f'<dt>Matched zero profiles</dt><dd>{_cov_zero}</dd>'
        f'<dt>Matched one profile</dt><dd>{_cov_single}</dd>'
        f'<dt>Matched multiple profiles</dt><dd>{_cov_multi}</dd>'
        f'</dl></div>'
    )
    ...
    # ── assemble page ──────────────────────────────────────────────────────────
    _top_n_actual = min(top_n, len(scored))
    page = (
        f'<!doctype html><html lang="en"><head>'
        f'<meta charset="utf-8">'
        f'<meta name="viewport" content="width=device-width,initial-scale=1">'
        f'<title>{title}</title>'
        f'<style>{_BUILTIN_CSS}{css_override}</style>'
        f'</head><body>'
        f'<header><h1>{title}</h1>'
        f'<span style="margin-left:auto;font-size:0.8rem;opacity:.8">{ts}</span></header>'
        f'<div class="container">'
        f'<div class="cards">{cards_html}</div>'
        f'{cov_html}'
        f'<h2 style="margin-bottom:0.75rem;font-size:1rem;">Top {_top_n_actual} Commits</h2>'
        f'{table_html}'
        f'{prof_html}'
        f'</div>'
        f'<footer>Generated by kcommit-analysis-pipeline {VERSION} · {ts}</footer>'
        f'</body></html>'
    )
