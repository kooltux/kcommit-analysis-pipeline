...
def main():
    ...
    cache = os.path.join(work, 'cache'); outdir = os.path.join(work, 'output'); ensure_dir(outdir)
    ...
    with open(os.path.join(outdir, 'relevant_commits.csv'), 'w', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(['commit','subject','product_score','security_score','performance_score','stable_score','candidate_score','matched_profiles','product_evidence'])
        for c in scored:
            w.writerow([c.get('commit'), c.get('subject'), c.get('product_score'), c.get('security_score'), c.get('performance_score'), c.get('stable_score'), c.get('candidate_score'), ';'.join(c.get('matched_profiles', [])), ';'.join(c.get('product_evidence', []))])
    save_json(os.path.join(outdir, 'relevant_commits.json'), scored)
    profile_summary = {'profiles': dict((p, len(scored)) for p in cfg.get('active_profiles', [])), 'overlaps': {','.join(cfg.get('active_profiles', [])): len(scored)} if cfg.get('active_profiles') else {}}
    ...
    finish_stage(state_path, 'report_commits', started, status='ok', extra={'reported_commit_count': len(scored), 'output_dir': outdir})
