...
| `hunk_count_chunk_size` | `2000` | Max SHAs per `git show` invocation when `count_hunks` runs. Guards against the OS `ARG_MAX` limit (`OSError: [Errno 7] Argument list too long`) on very large commit ranges (tens to hundreds of thousands of commits), where passing every SHA on a single command line would overflow the kernel's argument-list size. The SHA list is split into batches of at most this size, each diffed with a separate `git show` call; results are merged transparently. Rarely needs changing — lower it only on unusually constrained environments (e.g. very small `ARG_MAX`); raising it trades fewer subprocess spawns for a larger per-call argv. |
...
| `cherry_pick_cache_dir` | `null` | Directory for the SQLite cherry-pick result cache: one direct file per target revision, named after the revision with `/` and `\` normalized to `_` (`<dir>/<safe-rev_old>`, e.g. `<dir>/v6.1.1`). **Required** when `cherry_pick_test` is enabled — incremental runs reuse cached results instead of re-testing every commit (10-100x speedup). |
...
"collect": {
  ...
  "hunk_count_chunk_size": 2000, // max SHAs per git show invocation when counting hunks; guards against OS ARG_MAX overflow on very large commit ranges. Rarely needs changing.
  ...
  "cherry_pick_cache_dir": null, // REQUIRED when cherry_pick_test is enabled. SQLite cache dir; one direct file per target revision: <dir>/<safe-rev_old> (revision name with / and \ normalized to _, e.g. <dir>/v6.1.1). Only new commits are tested on reruns (10-100x speedup for incremental runs against released/immutable kernel history).
