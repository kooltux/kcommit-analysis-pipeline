 ...
 *
 * v18.3.0 — fetchCommit() updated for bucket shard layout (G.4, v18.1.0):
 *   Sidecar path changed from commits/<sha>.json (one file per commit)
 *   to commits/<sha[0]>/<sha[1:3]>.json (grouped shard dict).
 *   After fetching, the commit entry is looked up by full SHA with a
 *   sha12 fallback.  Embedded-store (STORE) fast-path is unchanged.
 */
