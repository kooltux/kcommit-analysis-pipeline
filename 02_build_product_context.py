...
def main():
    ...
    # Stage 1 in the new 0..4 / 1..5 scheme (was 3 and 4 in 7-stage scheme)
    started    = start_stage(state_path, 'build_product_context', 1, 5)

    try:
        # Validation is now also handled by the driver, but we keep it here for safety
        # if run standalone.
        problems, notices = validate_inputs(cfg)
        if problems:
            fail_stage(state_path, 'build_product_context', started,
        ...
        finish_stage(state_path, 'build_product_context', started, status='ok',
                     extra={
        ...
        import traceback
        traceback.print_exc()
        fail_stage(state_path, 'build_product_context', started, error_msg=str(exc))
        raise
